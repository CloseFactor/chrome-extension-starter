...
import {SessionMapNames, SidebarDisplayMode} from "../../shared/models/session-storage.model";
  ...
  chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    if (changeInfo.status === 'complete') {
      if (tab.url?.match(SAMPLE_PAGE_URL_REGEX)) {
        return displaySidebar({type: MessageType.SHOW_EXTENSION, url: '', tabId} as Message);
      }
    }
    return false;
  });
